  ...
  ws.on('message', message => {
    ...
    switch (client_data.type) {
      case 'initialMessage':
        timer = setInterval( () => {
          const query = `
            SELECT last_version, last_modificator, cards_side 
            FROM   sessions
            ...
            (err, results) => {
              if (err) { console.log('err', err) }
                ...
                const send_data = {
                  ...
                  cards_side:       results[0].cards_side
                }
      ...
      case 'saveCardThisSession':
        ...
        const {session_id, modificator, cards_id, cards_box, cards_name, cards_img, position_left, position_top, scale} = client_data
        ...
        promise.then(
          ...
          () => {
            const query_cards_1 = `
              ...
              VALUES (null, ${session_id}, '${cards_id}', ${cards_box}, '${cards_name}', '${cards_img}', ${position_left}, ${position_top}, ${scale})`
            const query_cards_2 = `
              UPDATE sessions 
      ...
      case 'setSelectedCards':
        ...
        break
      case 'setSelectedCardsSide':
        const query_setSide = `
          UPDATE sessions 
          SET    last_version = last_version + 1, cards_side = ${client_data.side}, last_modificator = '${client_data.modificator}'
          WHERE  session_id = ${client_data.session}`
        connection.query(
          query_setSide,
          (err, results) => {
